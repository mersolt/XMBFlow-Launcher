...
if (-not $text.Contains('submenu_selection = submenu_selection - 1') -or -not $text.Contains('submenu_selection = submenu_selection + 1') -or -not $text.Contains('local parent_x = category_anchor_x - 390 * submenu_alpha') -or -not $text.Contains('1 - 0.52 * submenu_alpha')) { throw 'The submenu must smoothly move and dim the parent axis while it owns Up/Down navigation.' }
...
if (-not $text.Contains('local x = parent_x + relative * 130')) { throw 'The horizontal category axis must interpolate around the submenu-adjusted XMB anchor.' }
