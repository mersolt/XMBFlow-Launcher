...
if (-not $text.Contains('local submenu_open = false') -or -not $text.Contains('local function draw_submenu_options(column)') -or -not $text.Contains('Controls.check(pad, SCE_CTRL_CROSS)') -or -not $text.Contains('submenu_open = true') -or -not $text.Contains('if submenu_open then')) { throw 'The standalone mockup must open a second read-only vertical submenu axis with Cross.' }
if (-not $text.Contains('submenu_selection = submenu_selection - 1') -or -not $text.Contains('submenu_selection = submenu_selection + 1') -or -not $text.Contains('draw_vertical_options(vertical_column, vertical_alpha * 0.48)')) { throw 'The submenu must own Up/Down navigation while the parent axis remains visible and dimmed.' }
