...
if (-not $text.Contains('local previous_column = 5') -or -not $text.Contains('local vertical_fade = 1') -or -not $text.Contains('draw_vertical_options(previous_column, 1 - vertical_fade)')) { throw 'Horizontal category changes must cross-fade the vertical object axis.' }
...
if (-not $text.Contains('math.sin(glow_phase / 18)')) { throw 'Focused objects must use the restrained XMB-style glow pulse.' }
