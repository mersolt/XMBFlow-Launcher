...
if (-not $text.Contains('XmbRender.each_vertical(1, option_counts[column], visual_option, 296, 66, 234') -or $text.Contains('local arc = 1 - math.abs(1 + relative * 2)')) { throw 'The preceding object must move behind the fixed category icon without a lateral detour.' }
