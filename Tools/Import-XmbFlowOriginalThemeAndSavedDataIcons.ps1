...
if ([string]::IsNullOrWhiteSpace($ProvenancePath)) { $ProvenancePath = Join-Path $PSScriptRoot '..\assets\user-provided-theme-and-saved-data-icons.json' }
...
$files = @(@{ input = 'theme-settings.png'; output = 'xmb-setting-theme.png' }, @{ input = 'saved-data-utility.png'; output = 'xmb-object-saved-data.png' })
$records = @()
...
[ordered]@{ schema = 1; provenance = 'User-provided original XMBFlow icon artwork; author asserted original work in project discussion on 2026-07-24.'; license = 'LicenseRef-XMBFlow-Original'; transformation = 'Tools/Import-XmbFlowOriginalThemeAndSavedDataIcons.ps1'; files = $records } | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $ProvenancePath -Encoding UTF8
Write-Host "Imported $($records.Count) user-provided original icons."
